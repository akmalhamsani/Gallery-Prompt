...
function updateCounts() { if(currentUserId){ const pendingUser = allItems.filter(i => i.userId === currentUserId && i.status === 'pending').length; if(pendingUser > 0) { els.counts.pending.textContent = pendingUser; els.counts.pending.classList.remove('hidden'); els.mobilePendingCount.textContent = pendingUser; els.mobilePendingCount.classList.remove('hidden'); } else { els.counts.pending.classList.add('hidden'); els.mobilePendingCount.classList.add('hidden'); } } const pendingAll = allItems.filter(i => i.status === 'pending').length; if(pendingAll > 0) { els.counts.request.textContent = pendingAll; els.counts.request.classList.remove('hidden'); els.mobileRequestCount.textContent = pendingAll; els.mobileRequestCount.classList.remove('hidden'); els.mobileNavRequest.classList.remove('hidden'); } else { els.counts.request.classList.add('hidden'); els.mobileRequestCount.classList.add('hidden'); els.mobileNavRequest.classList.add('hidden'); } }
function notify(msg, type='success') { els.notifMsg.textContent = msg; els.notifIcon.textContent = type==='error'?'⚠️':(msg.includes('komen')?'💬':(msg.includes('Like')?'❤️':(msg.includes('balas')?'↩️':'🔔'))); els.notif.className = `fixed bottom-5 left-1/2 -translate-x-1/2 px-5 py-3 rounded-lg shadow-lg text-sm font-medium z-[200] flex items-center gap-2 transition-all duration-300 transform translate-y-0 opacity-100 ${type === 'error' ? 'bg-red-600' : 'bg-gray-800'} text-white`; els.notif.classList.remove('hidden'); setTimeout(() => { els.notif.classList.add('translate-y-10', 'opacity-0'); setTimeout(()=>els.notif.classList.add('hidden'), 300); }, 3000); }
